...
function writePassword() {
  ...
  mySymbols = confirm(
    "Do you want your password to include symbols? Select 'ok' for yes"
  );
  //pasword numbers yes/no
  myNumbers = confirm(
    "Do you want to include numbers in your password? Select 'ok' for yes"
  );
  //password uppercase yes/no
  myAlpha = confirm(
    "Do you want upper case letters in your password? Select 'ok' for yes"
  );
  //password lowercase yes/no
  myBeta = confirm(
    "do you want lower case letters in your password? Select 'ok' for yes"
  );

  if (passSymbols) {
    passwordCharacters = passSymbols.concat(passwordCharacters);
    console.log(passSymbols);
  }
  if (passNumbers) {
    passwordCharacters = passNumbers.concat(passwordCharacters);
    console.log(passNumbers);
  }
  if (passAlpha) {
    passwordCharacters = passAlpha.concat(passwordCharacters);
    console.log(passAlpha);
  }
  if (passBeta) {
    passwordCharacters = passBeta.concat(passwordCharacters);
    console.log(passBeta);
  }
  var randomPassword = "";
